...
public class FreeCell extends JLayeredPane{
	...
	public void addMouseListeners(Card c) {
		...
		jc.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				ArrayList<Card> topOfPile = ((Pile)c.parent).getAlternatingSequence();
	...
	protected boolean isValidMove(Pile held, Pile from, Pile to) {
		...
		if(to == null || to == from) 
			return false;
		else if(utilPiles.indexOf(to) > -1 && heldPile.size()==1) {
			if(pilesIndexOf(utilPiles, to) < 4) {
				if(to.isEmpty()) return true;
				else return false;
			}
			else {
				if(to.isEmpty() && held.getFirst().rank != '1') return false;
				if(heldPile.getFirst().compareRank(to.getLast()) == -1 && heldPile.getFirst().isSameColor(to.getLast())) return true;
			}
			return false;
		}
		if (to.isEmpty()) return true;
