...
public class FreeCell extends JLayeredPane{
	...
	public FreeCell(){
		...
		utilPane = new JPanel(new GridLayout(1,COLS)) {
			...
			public Dimension getPreferredSize() {
				return new Dimension(getWidth(), (int) (getWidth()/COLS*JCard.getRatio()));
			}
		};
	...
	public void addMouseListeners(Card c) {
		...
		jc.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				// If foundation pile or about to touch to move, we skip this input to go to mouseReleased
				if(pilesIndexOf(utilPiles, (Pile)c.parent) > 3 || (selectedCard!=null && heldPile==null && ((Pile)selectedCard.parent)!=((Pile)c.parent))) {
					if(selectedCard!=null) ((Pile)selectedCard.parent).pilePane.unhighlightAllCards();
					return;
				}
				...
				ArrayList<Card> topOfPile = ((Pile)c.parent).getAlternatingSequence(getMoveLength());
			...
			public void mouseReleased(MouseEvent e) {
				...
				if(selectedCard!=null && heldPile==null) {
					...
					ArrayList<Card> topOfPile = ((Pile)selectedCard.parent).getAlternatingSequence();					
				...
				for (Pile p:utilPiles) 
					if(p.pilePane.getBounds().contains(SwingUtilities.convertPoint(((Pile)c.parent).cardsMap.get(c), e.getPoint(), utilPane))) 
						hoveringOver = p;
		...
		jc.addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				if(pilesIndexOf(utilPiles, (Pile)c.parent) > 3) return;
				...
				// Move it around according to where it it on the FreeCell panel in general
				Point p = SwingUtilities.convertPoint(((Pile)c.parent).cardsMap.get(c), e.getPoint(), FreeCell.this);
	...
	protected void checkPile(Pile p) {
		ArrayList<Card> top = p.getAlternatingSequence();
		if(top.size() > 12) {
			p.removeAll(top);
	...
	private int pilesIndexOf(ArrayList<Pile> piles, Pile p) {
